...
let show_box_1 = document.getElementsByClassName('show_box_1')[0];

audience_btn.addEventListener('click', ()=>{
    show_box_1.classList.toggle('show_box_active')
})
...
select_reply_btn.addEventListener('click', ()=> {
    who_can_reply_bx.classList.toggle('show_box_active');
})
...
Array.from(document.getElementsByClassName('who_can_reply')).forEach((i,a) => {
    i.addEventListener('click', ()=>{
        ...
        if(a === 0){
            select_reply_btn.innerHTML = `<i class="fa-solid fa-earth-asia">Everyone can reply`;
        }
        else if(a === 1){
            select_reply_btn.innerHTML = `<i class="fa-solid fa-user-check">People you follow`;
        }
        else{
